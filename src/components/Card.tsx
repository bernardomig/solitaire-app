...
const Container = styled.div<{ color: "red" | "black"; flipped?: boolean }>`
  display: flex;
  height: 200px;
  width: 135px;
  ...
  color: ${(props) => (props.color === "red" ? "#ce4648" : "#2C4D71")};
`;

const Symbols = styled.div<{ reversed?: boolean }>`
  display: flex;
...
export default function Card(
  ...
  if (args.variant === "default") {
    const { rank, suit } = args;
    const Icon = SUIT_ICONS[suit];

    return (
      <Container
        color={suit === Suit.Spade || suit === Suit.Club ? "black" : "red"}
      >
        <>
          <Symbols>
            <div>{rank}</div>
  ...
  } else if (args.variant === "flipped") {
    return <Container color="black" flipped={true}></Container>;
  } else if (args.variant === "placeholder") {
    return <Placeholder />;
  } else {
    throw Error("invalid card variant");
  }
}
